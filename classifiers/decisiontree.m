...
for i = 1:10
    test = (indices == i); 
    train = ~test;
    mdl = fitctree(Xtrain(train,:),Ytrain(train,:));
...
fprintf('K-Fold Cross Validation (k=10) with Decision Tree\n')
fprintf('Accuracy: %f\n',1- cp.ErrorRate);
fprintf('Sensitivity: %f\n',cp.Sensitivity);
fprintf('Specificity: %f\n',cp.Specificity);
fprintf('F1-score: %f\n\n',2/((1/cp.Sensitivity)+(1/cp.Specificity)));

% K-Fold Cross Validation (k=10) with Decision Tree
% GSE103334
% Accuracy: 0.435688
% Sensitivity: 0.439236
